...
<p>An agentic browser is a browser with an AI agent built into the core browsing engine, not bolted on as an extension. It can read a page, act across your open tabs, and complete multi-step tasks on your behalf. That means researching a topic, comparing products, filling out a form, or finishing a purchase. This is the defining shift of 2026's browser market. Chrome is adding Gemini, Edge has Copilot Mode, and Opera has Neon. But Atlas, Comet, and Dia are built from the ground up around agentic behavior, rather than added onto an existing browsing experience.</p>
<p>The reason this matters now, in mid-2026, is simple. All three products have moved from invite-only betas to public, free-to-download releases within the past year. Perplexity raised $200 million at a valuation near $20 billion in early June 2026. It says the funding will go toward expanding Comet's agent capabilities. Atlassian acquired The Browser Company, the maker of Dia, for $610 million in September 2025. This is no longer an experimental corner of the browser market. Real money and real user bases are behind all three.</p>

<h2 id="key-features">ChatGPT Atlas, Perplexity Comet, and Dia: key features</h2>
<h3>Agent mode and task automation</h3>
<p>Atlas's Agent Mode is the most careful of the three by design. It can research topics, plan events, compare items, and handle repetitive multi-step tasks. But OpenAI built in hard capability restrictions: Agent Mode cannot run code, install extensions, or download files. It also pauses on sensitive sites, like your bank's login page, to confirm you really intend the action. It's gated to ChatGPT Plus ($20/month) and higher; the free tier gets browsing and sidebar chat, not autonomous task execution.</p>
<p>Comet's agent leans research-first rather than task-first. It opens relevant sources, extracts information, and assembles a structured summary within the browsing session itself. That's really useful for journalists, analysts, and competitive-intelligence work. It's less oriented toward "book this for me" style completion tasks than Atlas.</p>
...
<p>Dia's standout feature here is Memory, which, per The Browser Company's CEO, "learns from every tab you open" and surfaces relevant facts later. It also auto-creates Tab Groups around calendar events. Open a meeting invite, and every tab you open during that call gets grouped and labeled automatically, then stays searchable afterward. This is the single most differentiated feature across the three browsers. It's also the one that drew the most direct privacy criticism on r/DiaBrowser.</p>
<p>Comet maintains context across tabs to support its research workflow. Some users report the browser losing track when too many tabs are open at once. That's a known criticism even in generally positive reviews. Atlas offers an optional memory system tied to your ChatGPT account history, with user controls to view, edit, or delete stored memories. OpenAI states memories aren't used to train its models unless you opt in.</p>

<h3>Privacy controls and data handling</h3>
<p>Each browser gives you some level of control over what its agent can see and store, but the defaults differ meaningfully. Atlas lets you view, edit, or delete stored memories from your account settings. OpenAI states that browsing memories aren't used to train its models unless you explicitly opt in. That's a distinction worth checking in your own account rather than assuming.</p>
...
<p>Dia is the outlier here. Its May 2025 privacy policy update expanded collection to include full Assistant query and response logs alongside the pages you visit. Unlike Atlas, there's no prominent per-item memory-deletion control surfaced in the interface as of this review. Granular control over what an AI browser remembers may matter more to you than the feature set itself. If so, that alone may decide which of the three is right for you.</p>
...
<p>In March 2026, researchers at Zenity Labs published a family of vulnerabilities called "PleaseFix," demonstrating zero-click agent hijacking in Comet. One path could access and extract credentials from a 1Password vault through the agent's own authorized workflows, without directly exploiting any flaw in 1Password. The mechanism runs through the browser's execution architecture. Comet, like Atlas and Dia, is Chromium-based and uses a privileged browser extension that accepts task commands through an API. It observes pages either through the page's underlying structure or through screenshots converted to image tokens.</p>
<p>This isn't a reason to write off agentic browsers entirely. Atlas's hard guardrails exist for a reason: no code execution, no extension installs, no downloads, confirmation prompts on sensitive sites. OpenAI built them in because it's aware of this exact risk category. But it is a reason to be deliberate. Don't grant an agentic browser standing access to a password manager, banking session, or email account until you've read what that product's agent is authorized to do. Keep the browser updated as vendors patch these issues.</p>
<p>Dia's risk profile is different in kind. It's less about a known exploit and more about the sheer scope of what it collects by default. The May 2025 privacy policy update flagged on r/DiaBrowser expanded data collection to include the pages you visit. It also covers the full content of your queries to, and responses from, the Dia Assistant. That's a data-retention decision, not a security bug, but it matters just as much when deciding what to install.</p>
...
<h3>Best for</h3>
<p><strong>Choose Perplexity Comet if:</strong> you do research-heavy work (writing, analysis, competitive intelligence) and want citation-backed answers without paying anything for the agentic features. <strong>Choose ChatGPT Atlas if:</strong> you already pay for ChatGPT Plus or higher and want task automation with careful, well-known guardrails. <strong>Choose Dia if:</strong> you're on an Apple Silicon Mac, live inside a lot of scheduled meetings, and want automatic tab organization around them. Just make sure you've read and accepted its data-collection scope first.</p>
<h3>Skip if</h3>
