...
<p><strong>Claude Code</strong> is Anthropic's terminal-based agentic coding tool, and it remains the developer consensus pick for raw multi-file reasoning on large codebases. But "the best coding agent" and "the only terminal agent worth running" stopped being the same claim sometime in early 2026. That's when OpenAI open-sourced a Rust rewrite of Codex and Google shipped a fresh CLI to replace the retiring Gemini CLI. Around the same time, the open-source OpenCode project quietly overtook Claude Code's own GitHub star count.</p>
<p>None of this makes Claude Code a bad tool. It has no free tier, and Pro starts at $20/month with usage limits that reset every 5 hours. Developers doing heavy agentic refactors regularly report burning through a session's allowance in under an hour. That combination, no free option, hard usage ceilings, and a growing field of credible competitors, is exactly what sends people searching for <strong>Claude Code alternatives</strong>.</p>
...
<p>The cost story is what puts it at the top of this list. The CLI itself is free to download and run against your own OpenAI API key. Separately, Codex usage is bundled into every ChatGPT tier from Free up through Enterprise, with usage limits that scale by plan. Most developers already paying for ChatGPT Plus get real Codex CLI capacity at no additional cost. On the current Terminal-Bench 2.1 leaderboard, Codex CLI running GPT-5.5 sits at the top of the ranked table. That puts it ahead of Claude Code's best usable Opus 4.8 pairing.</p>

<h3>2. OpenCode: Best Free, Fully Open-Source, Model-Agnostic Pick</h3>
<p>OpenCode is the open-source coding agent built by the team behind the SST serverless framework. It has become the default answer for developers who do not want to commit to a single model vendor's terminal tool. It is MIT-licensed and has passed 160,000 GitHub stars. It connects to more than 75 model providers, including Anthropic, OpenAI, Google, AWS Bedrock, Groq, OpenRouter, and fully local models via Ollama.</p>
<p>Because OpenCode is bring-your-own-key, its actual monthly cost is whatever the underlying model calls cost, including $0 if you route to a local Ollama model. That flexibility is the entire pitch. You can start a session on a cheap model for exploration. Then switch to Claude Opus or GPT-5.5 only for the parts of a task that need frontier-level reasoning, all inside the same tool.</p>

<h3>3. Aider: Best for a Git-Native Workflow</h3>
<p>Aider is the longest-running tool on this list, and its defining feature has not changed. Every AI-made edit is committed to git automatically with a descriptive commit message. That turns your normal git log into a reviewable, revertible audit trail of everything the agent did. It is free, open source under Apache 2.0, and works with any model provider you configure. There is no vendor lock-in and no bundled subscription to manage.</p>
...
<p>The important caveat: Antigravity CLI is closed source. Gemini CLI, by contrast, was fully open source under Apache 2.0, with over 100,000 GitHub stars and thousands of external contributions. It is free during its current preview period and carries over Gemini CLI's Agent Skills, Hooks, Subagents, and Extensions. Several independent testers have reported daily free-request limits considerably tighter than Gemini CLI's old 1,000-requests-per-day allowance. Treat it as the current Google-native option, not necessarily a stable long-term free tier.</p>

<h3>5. Cursor CLI: Best If You Already Pay for Cursor</h3>
<p>Cursor launched a genuine terminal agent, Cursor CLI, in January 2026, extending its AI-native code editor into the command line. A Cloud Handoff feature lets a terminal session and an IDE session share the same agent state. If you are already on Cursor Pro ($20/month) for the editor, Cursor CLI is effectively a free extension of a subscription you are already paying for. It gives you access to Claude, GPT, and Cursor's own models from one billing relationship.</p>
...
<p>Warp takes a different approach entirely. Instead of being one agent, it replaces your terminal application. It lets you run Claude Code, Codex, and Warp's own agent side by side inside the same interface, each in its own tab. Warp's terminal core is open source. Pricing runs Free ($0, with a limited monthly AI credit allowance) and Build ($20/month, 1,500 credits plus bring-your-own-key support for OpenAI, Anthropic, or Google). Business runs $50/user/month with SSO and enforced zero data retention.</p>
...
<p>Amazon Q Developer CLI still technically works, with a free tier (50 agentic requests/month) and a $19/month Pro tier. But AWS closed new signups for both tiers on May 15, 2026. It has confirmed the IDE plugins and CLI reach end of support on April 30, 2027. AWS is steering new developers toward <strong>Kiro</strong>, its newer spec-driven agentic development environment, instead. If you already have an active Q Developer subscription, you can keep using it and adding seats. But it is not a tool worth setting up from scratch in mid-2026.</p>

<h2>Terminal-Bench and SWE-bench: What the Benchmarks Actually Show</h2>
<p>Two benchmarks dominate how these agents get compared publicly. <strong>Terminal-Bench 2.1</strong> scores an agent-plus-model pairing on real, multi-step terminal tasks. That's not just code generation, but the full loop of reading a repo, planning, executing shell commands, and verifying the result. <strong>SWE-bench Pro</strong>, maintained by Scale AI, is a contamination-resistant set of real GitHub issues. It's used to measure how often a model resolves an actual reported bug or feature request end to end.</p>
<p>On the most recent verified Terminal-Bench 2.1 snapshot, Codex CLI running GPT-5.5 topped the leaderboard. Claude Code on Opus 4.8 was the strongest usable Claude pairing, close behind. On SWE-bench Pro, the ranking flips. Claude Opus 4.8 leads at roughly 69%, a meaningful jump from Opus 4.7's score, ahead of GPT-5.5 and Gemini 3.1 Pro. The takeaway is not that one model is simply "better," but that benchmark leaders shift between task types. A ranking based on one leaderboard alone will miss half the picture.</p>
<p>It is also worth reading these scores as agent-plus-model pairs, not model scores in isolation. Every bring-your-own-key tool on this list, OpenCode, Aider, Warp, inherits whatever model you point it at. So an OpenCode session running Claude Opus 4.8 should perform close to Claude Code running the same model on the underlying reasoning. That's true even though the two tools built around it differ in workflow.</p>

<h2>Free vs Paid: What You Actually Get Without a Credit Card</h2>
<p>If budget is the deciding factor, three tools stand out for genuinely free use rather than a time-limited trial. <strong>OpenCode</strong> and <strong>Aider</strong> are free indefinitely because they are bring-your-own-key. You only pay for the model tokens you consume, which can be $0 on a local Ollama model. <strong>OpenAI Codex CLI</strong> is free as a tool and comes bundled with every ChatGPT tier, including the Free plan. Usage limits do scale down at the lowest tiers.</p>
<p><strong>Antigravity CLI</strong> and <strong>Warp's</strong> base tier are also free, but both come with caveats. Antigravity's free-tier limits are reportedly tighter than the CLI it replaced. Warp's free tier includes a limited monthly AI credit allowance, not unlimited agent usage. Claude Code and Amazon Q Developer CLI's Pro tier are the only two options here with no meaningful free path at all. Q Developer's free tier still exists for existing users, but new signups are closed entirely.</p>

<h2>What Actually Transfers When You Switch Tools</h2>
<p>Switching terminal agents used to mean rebuilding your setup from scratch. Custom prompts, project instructions, and workflow automations were locked to whichever tool you started with. That has changed for two specific pieces of configuration. <strong>SKILL.md</strong> is a reusable-workflow format built around a project-level instructions file. It's now recognized as an open standard across Claude Code, OpenCode, Codex CLI, and Antigravity CLI. A skill file written for one tool generally works unmodified in another.</p>
...
<div style="margin:30px 0 12px;padding:16px;border:1px solid rgba(13,148,136,.25);background:rgba(13,148,136,.06);border-radius:12px;">
  ...
  <div style="display:inline-block;"><a href="https://developers.openai.com/codex/cli" target="_blank" rel="sponsored nofollow noopener noreferrer" style="display:inline-block;background:linear-gradient(135deg,#0D9488,#0f766e);color:#fff;padding:10px 14px;margin:6px 8px 0 0;border-radius:10px;font-weight:700;font-size:13px;text-decoration:none;">Visit developers</a></div>  <div style="display:inline-block;"><a href="https://platform.claude.com/docs/en/about-claude/pricing" target="_blank" rel="sponsored nofollow noopener noreferrer" style="display:inline-block;background:linear-gradient(135deg,#0D9488,#0f766e);color:#fff;padding:10px 14px;margin:6px 8px 0 0;border-radius:10px;font-weight:700;font-size:13px;text-decoration:none;">Visit platform</a></div>  <div style="display:inline-block;"><a href="https://developers.googleblog.com/an-important-update-transitioning-gemini-cli-to-antigravity-cli/" target="_blank" rel="sponsored nofollow noopener noreferrer" style="display:inline-block;background:linear-gradient(135deg,#0D9488,#0f766e);color:#fff;padding:10px 14px;margin:6px 8px 0 0;border-radius:10px;font-weight:700;font-size:13px;text-decoration:none;">Visit developers</a></div>  <div style="display:inline-block;"><a href="https://aider.chat/" target="_blank" rel="sponsored nofollow noopener noreferrer" style="display:inline-block;background:linear-gradient(135deg,#0D9488,#0f766e);color:#fff;padding:10px 14px;margin:6px 8px 0 0;border-radius:10px;font-weight:700;font-size:13px;text-decoration:none;">Visit Aider: Official Site & Docs</a></div>  <div style="display:inline-block;"><a href="https://opencode.ai/" target="_blank" rel="sponsored nofollow noopener noreferrer" style="display:inline-block;background:linear-gradient(135deg,#0D9488,#0f766e);color:#fff;padding:10px 14px;margin:6px 8px 0 0;border-radius:10px;font-weight:700;font-size:13px;text-decoration:none;">Visit OpenCode: Official Site</a></div>  <div style="display:inline-block;"><a href="https://aws.amazon.com/blogs/devops/amazon-q-developer-end-of-support-announcement/" target="_blank" rel="sponsored nofollow noopener noreferrer" style="display:inline-block;background:linear-gradient(135deg,#0D9488,#0f766e);color:#fff;padding:10px 14px;margin:6px 8px 0 0;border-radius:10px;font-weight:700;font-size:13px;text-decoration:none;">Visit aws</a></div>
