...
<p>Relevance AI splits cost into two meters: Actions (what your agent does) and Vendor Credits (the underlying model cost, passed through at zero markup). The <strong>Free</strong> plan gives 200 Actions/month plus a one-time 1,000 Vendor Credits.</p>
...
<p>Gumloop looks and feels closer to Zapier or Make than to a chatbot. It's a drag-and-drop canvas where each node represents a step. AI calls (GPT, Claude, Gemini) are first-class node types alongside scraping, API calls, and data transforms. This makes it the strongest of the three for structured, multi-stage data work: scrape a source, summarize with AI, extract structured fields, write to a database. Each stage needs to feed cleanly into the next.</p>
...
<p>Consider a common small-business use case. An agent researches 200 inbound leads per month, drafts a personalized outreach email for each, and logs the result to a CRM. That's three AI-touching steps per lead, 600 AI actions total per month. On <strong>Relevance AI's</strong> Pro plan, each step consumes one Action. 600 Actions/month sits comfortably within the 2,500-Action allowance at $19/month, with Vendor Credits covering the underlying model cost.</p>
<p>On <strong>Gumloop</strong>, the same workflow built as three nodes per lead uses roughly 2–20 credits per AI node, depending on model tier. That lands somewhere between 1,200 and 12,000 credits/month, and likely requires the $37/month Pro tier once research and drafting both use higher-tier models.</p>
